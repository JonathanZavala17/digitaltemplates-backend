...
class Planes extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->Model("PlanesModel","planes");
    ...
    function get_data(){
        ...
        $row = $this->planes->get_all_rows($order, $search, $valid_columns, $length, $start, $dir);
        ...
        if ($row != 0) {
            ...
            foreach ($row as $rows) {
                ...
                $filename = base_url() . "planes/editar";
                $menudrop .= "<a class='dropdown-item' href='" . $filename . "/" .$rows->id_plan. "' ><i class='fe-edit' ></i> Editar</a>";
                ...
                $menudrop .= "<a class='dropdown-item state_change' data-state='$txt'  id=" . $rows->id_plan. " ><i class='$icon'></i> $txt</a>";

                $menudrop .= "<a class='dropdown-item delete_row'  id=".$rows->id_plan." ><i class='fe-trash'></i> Eliminar</a>";
    ...
    function agregar(){
        if($this->input->method(TRUE) == "GET"){
            $category = $this->planes->get_categorias();
            $data = array(
              "rows"=>$category,
            );
            $extras = array(
            ...
            layout("planes/planes_agregar",$data,$extras);
        }
