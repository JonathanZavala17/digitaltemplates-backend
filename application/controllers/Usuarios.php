...
class Usuarios extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $extras = array(
            'css' => array(),
            'js' => array(
                0 => "js/funciones/user_functions.js"
            ),
        );
        ...
        if($this->input->method(TRUE) == "GET"){
            ...
            if ($_FILES["fileinput"]["name"] != "") {
                ...
                if ($this->upload->do_upload('file')){
                    $url = 'assets/admin/images/users/'.$name;
                    $data = array(
                        "first_name"=>$fname,
                        "last_name"=>$lname,
                        "username"=>$username,
                    ...
                    if($exits==0){
                        ...
                        if($insert){
                            ...
                            $xdatos['type'] = 'success';
                            $xdatos['title'] = 'Éxito';
                            $xdatos['msg'] = 'Usuario agregado exitosamente!';
                        }else{
                            $this->UtilsModel->rollback();
                            $xdatos['type'] = 'error';
                            $xdatos['title'] = 'Error';
                            $xdatos['msg'] = 'No se pudo guardar el usuario!';
                        }
                    }else{
                        $this->UtilsModel->rollback();
                        $xdatos['type'] = 'warning';
                        $xdatos['title'] = 'Advertencia';
    ...
    function editar($id_user=-1){
        if($this->input->method(TRUE) == "GET"){
            ...
            layout("config/edit_user",$data,$extras);
        }
        else if($this->input->method(TRUE) == "POST"){
            ...
            if ($_FILES["fileinput"]["name"] != "") {
                ...
                if ($this->upload->do_upload('file')){
                    ...
                    if($update){
                        ...
                        $xdatos['type'] = 'success';
                        $xdatos['title'] = 'Éxito';
                        $xdatos['msg'] = 'Usuario editado exitosamente!';
                    }else{
                        $this->UtilsModel->rollback();
                        $xdatos['type'] = 'error';
                        $xdatos['title'] = 'Error';
                        $xdatos['msg'] = 'No se pudo editar el usuario!';
                    }
                }else{
                    $xdatos["type"]="error";
                    $xdatos['title']='Alerta';
    ...
    function permisos($id_user=-1){
        if($this->input->method(TRUE) == "GET"){
            ...
            layout("config/permissions",$data,$extras);
        }
        else if($this->input->method(TRUE) == "POST"){
            $id_user = $this->input->post("id_user");
