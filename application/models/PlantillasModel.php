...
class PlantillasModel extends CI_Model
{
    ...
    {
        $this->db->select('plantillas.*, categorias.nombre as categoria');
        if ($order !=	 null) {
        ...
        $this->db->where('plantillas.estado !=', 'Borrado');
        $this->db->join('categorias', 'plantillas.id_categoria = categorias.id_categoria', 'left');
